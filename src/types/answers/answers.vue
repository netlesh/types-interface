<template>
  <div id="answers">
    <v-input
      v-model="answers.frage"
      :value="answers.frage"
      placeholder="Mehrere Antworten stimmen auf diese Frage"
    ></v-input>
    <div class="rightanswer">
      <h3>Antwortoptionen (bitte richtige markieren):</h3>

      <v-checkbox
        v-for="(item, index) in answers.optionen"
        :key="item"
        :value="item"
        :label="item"
        v-model="answers.correct"
      >
...
export default {
  ...
  methods: {
    addAnswer() {
      this.answers.optionen.push(this.neueOption);
      this.neueOption = "";
    },
    deleteAnswer(index) {
      const option = this.answers.optionen[index]
      this.answers.correct.splice(this.answers.correct.indexOf(option), 1)
      this.answers.optionen.splice(index, 1);
    },
  },
};
</script>
